...
class waJoinValidation {
    static submitUrl = Joi.object({
        url: Joi.string().uri().pattern(/^https:\/\/chat\.whatsapp\.com\/[a-zA-Z0-9]+$/).required()
    });

    static addGroup = Joi.object({
        userKey: Joi.string().required().label('User Key'),
        url: Joi.string().uri().pattern(/^https:\/\/chat\.whatsapp\.com\/[a-zA-Z0-9]+$/).required().label('URL'),
...
module.exports = waJoinValidation;
